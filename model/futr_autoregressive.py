...
class Autoregressivefc(nn.Module):
    def __init__(self, input_dim, n_class):
        super(Autoregressivefc, self).__init__()
        self.n_class = n_class
        
        # Define the final fully connected layer which takes input + previous output
        self.fc = nn.Linear(input_dim + n_class, n_class)

    def forward(self, tgt):
        batch_size, seq_len, input_dim = tgt.size()
        output_class = []
        prev_output = torch.zeros(batch_size, self.n_class).to(tgt.device)  # Initial previous output (zeros)

        for t in range(seq_len):
            # Concatenate input at time step t with previous output
            combined_input = torch.cat((tgt[:, t, :], prev_output), dim=-1)

            # Predict output for this time step
            current_output = self.fc(combined_input)

            # Append the current output to the output list
            output_class.append(current_output.unsqueeze(1))
            
            # Set current output as previous for the next step
            prev_output = current_output
        
        # Stack predictions along the sequence length
        output_class = torch.cat(output_class, dim=1)
        return output_class
...
class FUTR(nn.Module):
    ...
    def __init__(self, n_class, hidden_dim, src_pad_idx, device, args, n_query=8, n_head=8,
        ...
        if args.anticipate :
            ...
            self.fc_autoreg = Autoregressivefc(hidden_dim, n_class)
            # nn.init.xavier_uniform_(self.fc.weight)
            nn.init.xavier_uniform_(self.fc_autoreg.fc.weight)
    ...
    def forward(self, inputs, detections=None, target_nodes=None, mode='train'):
        ...
        if self.args.anticipate :
            ...
            if self.diffusion:
                ...
                output_classes = [self.fc_autoreg(t) for t in tgt]
                ...
                output_class = self.fc_autoreg(tgt) #[T, B, C]
